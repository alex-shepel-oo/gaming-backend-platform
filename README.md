...
scripts/k8s-apply.sh infra/kubernetes/gateway
...
`gateway/` is applied through `scripts/k8s-apply.sh` rather than a plain
`kubectl apply -f`: its ConfigMap is generated by Kustomize directly from
`backend/ApiGateway/ocelot.Kubernetes.json`, so there's no hand-copied
routing table that can drift from the real one. That file lives outside
`infra/kubernetes/gateway/`, though, and `kubectl apply -k` has no flag to
let Kustomize read outside the kustomization's own directory — only the
separate `kubectl kustomize` render command does — so the script renders
with `kubectl kustomize --load-restrictor=LoadRestrictionsNone` first and
pipes the result into `kubectl apply -f -`. The same script applies the
whole tree at once (`scripts/k8s-apply.sh infra/kubernetes`), Secrets
excluded — those stay the manual step above.
