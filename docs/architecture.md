...
| Mailpit | Always present (`mailpit` service) | Only in local kind clusters and the sandbox namespace, gated by `mailpit.enabled` (`true` in `values-local.yaml`, `false` in `values-production.yaml`). The production release never gets this Deployment; `Email__Smtp__Host` there points at the real relay |
