...
public sealed class EmailVerificationRequestedConsumerTests(RabbitMqFixture rabbitMq) : IDisposable
{
    private static readonly TimeSpan DeliveryTimeout = TimeSpan.FromSeconds(15);

    private readonly string _templatesDirectory = Directory.CreateTempSubdirectory("email-verification-consumer-tests-").FullName;

    public void Dispose() => Directory.Delete(_templatesDirectory, recursive: true);

    [Fact]
    public async Task EmailVerificationRequested_Delivered_RendersTemplateAndSendsWithExpectedContent()
    {
        ...
        var queueName = $"gbp.email.verification-requested.test.{Guid.NewGuid():N}";

        await using var connection = new RabbitMqConnection(MsOptions.Create(rabbitMqOptions));
        using var consumer = new EmailVerificationRequestedConsumer(
            connection,
            ...
            emailSender,
            NullLogger<EmailVerificationRequestedConsumer>.Instance,
            queueName);
        ...
        try
        {
            ...
            await PublishAsync(
                ...
                new { Email = "player@example.com", Code = "654321", GameName = "Test Game", ExpiresInMinutes = 20 },
                cancellationToken);
        ...
        sent.Subject.Should().Be("Confirm your email");
        sent.HtmlBody.Should().Contain("Test Game").And.Contain("654321").And.Contain("20");
        sent.TextBody.Should().Contain("654321");
    }
